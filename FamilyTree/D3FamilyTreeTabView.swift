...
// MARK: - PDF Generation
    private func generatePDF() async {
        ...
        print("📸 Capturing SVG from WebView...")
        let js = """
        (function() {
            try {
...
guard let dataURL = dataURL,
            ...
            print("❌ Failed to decode SVG data")
            return
        }
        
        print("✅ SVG captured, rendering to image...")
    ...
    private func createPDFWithImage(_ image: UIImage) -> Data? {
        ...
        return renderer.pdfData { context in
            ...
            let stats = "\(viewModel.people.count) people • \(viewModel.relationships.count) relationships • \(viewModel.layoutType.rawValue.capitalized)"
...
class D3TreeViewModel: ObservableObject {
    ...
    func loadFamilyData() async {
        print("🔄 D3 Tree: Starting loadFamilyData - isUsingMock: \(dataSourceManager.isUsingMockData)")
        do {
            let isUsingMock = dataSourceManager.isUsingMockData
            if isUsingMock {
                print("🎭 D3 Tree: Loading mock data...")
                ...
                print("✅ D3 Tree: Mock data loaded: \(mockPeople.count) people, \(mockRelationships.count) relationships")
                ...
                print("💾 D3 Tree: Loading from Supabase...")
                ...
                let relationshipsData: [RelationshipRecord] = try await SupabaseManager.shared.client
                    ...
                    .execute()
                    .value
                print("✅ D3 Tree: Supabase data loaded: \(peopleData.count) people, \(relationshipsData.count) relationships")
                people = peopleData
                relationships = relationshipsData
            }
            print("✅ D3 Tree: Data loaded successfully")
        } catch {
            print("❌ D3 Tree: Error loading family data: \(error)")
        }
    }
    ...
    private func buildHierarchicalJSON() -> String {
        guard let root = findRoot() else {
            print("⚠️ D3 Tree: No root person found")
...
svg { display: block; }
.node circle { fill: #fff; stroke: #007AFF; stroke-width: 1px; cursor: pointer; transition: all 0.3s; }
.node circle:hover { fill: #007AFF; stroke: #0051D5; stroke-width: 2px; }
