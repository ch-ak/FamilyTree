...
final class CleanPersonFormViewModel: ObservableObject {
    ...
    private func handleConfirmationResponse(input: String) async {
        ...
        // ✅ NEW: Handle existing family confirmation
        if pending.relationships.contains("existing_family") {
            if input.contains("yes") {
                // User confirmed the existing family is correct
                ...
                do {
                    let existingParents = try await repository.fetchRelatedPeople(sourceId: pending.person.id, relationshipType: .parent)
                    let existingSpouse = try await repository.fetchRelatedPeople(sourceId: pending.person.id, relationshipType: .spouse)
                    
                    if existingParents.isEmpty {
                        currentStep = .enterMother
                        appendSystemMessage("Let's start with your mother. What is her full name and year of birth?")
                        ...
                        // Has parents and spouse, ask for more siblings or children
                        currentStep = .enterSiblings
                        appendSystemMessage("Would you like to add more siblings? Enter one sibling's full name and birth year, or leave both blank to skip.")
                    }
                } catch {
                    appendSystemMessage("Error checking relationships: \(error.localizedDescription)")
                }
                
                clearInputs()
                return
            } else if input.contains("no") {
                // User says the existing info is wrong
                awaitingConfirmation = false
                pendingPerson = nil
                selfPersonId = nil
                selfDisplayName = nil
    ...
    private func handleEnterSelf(name: String, birthYear: Int) async throws {
        ...
        // Check if this person already exists with relationships
        let repository = DataSourceManager.shared.getCurrentRepository()
        ...
        if !existingParents.isEmpty || !existingSpouse.isEmpty || !existingSiblings.isEmpty {
            // Person already has relationships
            ...
            // Store the relationships for confirmation
            if !existingParents.isEmpty {
                motherId = existingParents.first?.id
                if existingParents.count > 1 {
                    fatherId = existingParents[1].id
                }
            }
            ...
            // New person or existing person with no relationships
    ...
    private func handleEnterSiblings(name: String, birthYear: Int) async throws {
        ...
        // ✅ CRITICAL FIX: Only link sibling to parents if they don't already have parents
        let repository = DataSourceManager.shared.getCurrentRepository()
        let siblingParents = try await repository.fetchRelatedPeople(sourceId: sibling.id, relationshipType: .parent)
    ...
    private func handleEnterChildren(name: String, birthYear: Int) async throws {
        ...
        // Link child to self (child points to parent via parent relationship)
        try await useCase.linkParent(childId: child.id, parentId: parentId)
        
        // Link child to spouse if spouse exists
